...
5. On `cache_expired`: an unknown number of messages were lost — do a full state resync via REST instead (e.g. [get unmatched orders](/pages/rest/user/get-unmatched-orders) and [get matched bets](/pages/rest/user/get-matched-bets)), then anchor your cursor at the tail of `availableMessages`. If nothing was retained, **clear the cursor and resync again on the next reconnect** — replaying from a cursor older than the snapshot would double-apply events.
6. Drain the buffer, **skipping any message whose `messageID` is not greater than your cursor** (those were already applied via replay), then go live. If you fell back to a full resync, **drop the buffer instead** — the REST snapshot already reflects those events — and advance your cursor to the newest buffered `messageID` if that is newer than your anchor. The cursor must only ever move forward.
...
// across process restarts, not just socket drops. null means "no valid
// replay anchor" — nothing to replay on a fresh start, full resync on a
// reconnect (see hasState below).
let cursor = null;
// True once any state has been applied (messages or a REST snapshot).
let hasState = false;
...
function advanceCursor(messageID) {
  if (isAfterCursor(messageID)) cursor = messageID;
}
...
async function replayGap() {
  ...
  hasState = true;
  // Anchor the cursor at the retained tip. If nothing was retained there
  // is no valid anchor — clear the cursor so the next reconnect resyncs
  // again instead of replaying from a stale position.
...
function connect() {
  ...
  ws.on('open', () => {
    openTask = (async () => {
      ...
      try {
        if (cursor !== null) {
          resynced = await replayGap();
        } else if (hasState) {
          // We hold state but have no valid replay anchor (a previous
          // resync could not establish one) — resync rather than risk
          // missing the gap. A truly fresh start has nothing to recover.
        ...
        await fullResyncViaRest();
        hasState = true;
        // The old cursor predates the snapshot — replaying from it later
        // would double-apply events the snapshot already covers.
        cursor = null;
        resynced = true;
      } finally {
        ...
        if (resynced) {
          ...
          // Just advance the cursor to the buffer tip if that is newer
          // than the anchor the resync established.
          if (pending.length > 0) {
            advanceCursor(pending[pending.length - 1].messageID);
          }
        } else {
