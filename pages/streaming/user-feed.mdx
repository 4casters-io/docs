...
6. Drain the buffer, **skipping any message whose `messageID` is not greater than your cursor** (those were already applied via replay), then go live.
...
function connect() {
  ...
  ws.on('open', async () => {
    console.log('user feed connected');
    if (cursor === null) {
      live = true; // first ever connect: nothing to replay
      return;
    }
    try {
      await replayGap();
