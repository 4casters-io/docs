...
| `filled` | number | always | **On a fill: the risk matched by this event alone**, not the running total. `0` on a placement. On a cancellation it is instead the cumulative risk matched before the cancel landed. |
...
<Warning>
  `filled` and `remaining` are not the same kind of number. `remaining` is running state — what is
  left on the book right now — but on a fill `filled` describes only that event. Across several
  partial fills they do not add up: `filled + remaining` equals `offered` on the first fill and
  drifts apart after that.

  To track how much of a resting order has been matched, use `offered - remaining`, or accumulate
  `filled` yourself. Do not mix the two.
</Warning>
