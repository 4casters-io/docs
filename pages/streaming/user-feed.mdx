...
function connect() {
  ...
  ws.on('open', () => {
    openTask = (async () => {
      console.log('user feed connected');
      try {
        if (cursor === null) await resync(); // first run, or no valid anchor
        else await replayGap();
      } catch (e) {
        console.error('replay failed, resyncing:', e.message);
        await resync();
      } finally {
        // Drain messages buffered during replay/resync. processMessage
        // skips anything the replay or snapshot already covered.
