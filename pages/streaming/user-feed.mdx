...
One overlap is unavoidable: a message that lands while the snapshot is being taken may also be reflected in it (REST responses carry no stream position). Handle notifications idempotently — dedupe fills by `txID`, orders by `orderID`.
