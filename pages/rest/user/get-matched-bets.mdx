...
<ResponseField name="data.matchedBets" type="array">
  ...
  <Expandable title="MatchedBet">
    ...
    <ResponseField name="risk" type="string">Risk on the matched portion (string-formatted, 2 decimals). Includes the commission you'd pay on a loss.</ResponseField>
    <ResponseField name="win" type="string">Win amount net of the commission you'd pay on a win. There is no separate `fee` field on matched bets — commission is charged at grading and only appears on graded wagers; until then it is reflected in `risk`/`win`.</ResponseField>
