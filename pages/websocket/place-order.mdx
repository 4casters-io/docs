...
<Note>
  A residual smaller than the **$10** minimum order size counts as complete: a `fillOrKill` for $1,000 that matches $992 succeeds, because the $8 that could not be filled is below the size that could ever rest on the book. Anything larger than $10 unmatched rejects the order.
</Note>
