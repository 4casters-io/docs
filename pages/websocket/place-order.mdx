...
<ResponseField name="orderType" type="string" required>
  One of `limit`, `post`, `postArb`, `fillAndKill`, or `fillOrKill`. See [Order types](#order-types) below.
</ResponseField>
...
A **partial** fill is a successful outcome: send `bet: 1000` into $400 of matchable liquidity and you are filled for $400 with the remaining $600 cancelled — a success, not an error. The order is rejected (`"fill and kill has no matches"`) only when there is no matchable liquidity at all. See the Fill And Kill scenarios in the [examples](#examples) below.
...
Rejections carry `errorType: rejected_order_type_rules` with one of two messages, distinguishing "nothing was there" from "not enough was there":
...
<Note>
If the socket drops before the response arrives, the place is **indeterminate, not failed** — it may still have been applied. Send a unique `userReference` on every order and reconcile with [`GET /user/getBetByUserReference`](/pages/rest/user/get-bet-by-user-reference) on reconnect rather than assuming the order never landed. See [Handling timeouts and indeterminate places](/pages/rest/orders/place-order#handling-timeouts-and-indeterminate-places) for the full pattern.
</Note>
...
### Scenario 5: Fill or Kill order that cannot be filled in full
...
- `error_type` is an enum you can rely on for programmatic handling:
    ...
    - `rejected_order_type_rules` — order-type rules forbid execution (e.g., `fillAndKill` found no executable liquidity, so nothing could be filled and the order was rejected; `fillOrKill` could not be filled in full; `post` found matches when matches are not allowed; or `postArb` was rejected because your American odds differ from the resting order you would match by **more than 1%**).
