...
**There are no API keys.** You authenticate with the same username and password you use to log in at 4casters.io, and all three APIs share the one token that login returns. API keys are planned; see [Authentication](/pages/authentication) for the current state.
...
Tokens are valid for 30 days and are rotated automatically after that. See [Authentication](/pages/authentication) for token lifetime, error handling, and credential guidance, and the [REST login reference](/pages/rest/authentication) for the full response shape.
