...
<CardGroup cols={3}>
  ...
  <Card title="订单 WebSocket" icon="bolt" href="/zh/pages/websocket/introduction">
...
1. 在 REST API 上使用用户名和密码调用 [`POST /user/login`](/zh/pages/rest/authentication)。
2. 在后续每个请求中发送返回的令牌：
   - **REST**：`Authorization: Bearer <token>` 请求头。
   - **WebSockets**：握手时使用 `Authorization: <token>` 请求头。

令牌有效期为 30 天，可在过期前刷新。令牌刷新、签名 Cookie 认证及其他细节见 [认证](/zh/pages/rest/authentication)。

## 约定 {#conventions}

- **传输**：仅 HTTPS / WSS。
- **编码**：一律使用 JSON。
- **响应信封（REST）**：成功时为 `{ "data": <payload> }`；HTTP 错误时为 `{ "error": "<message>" }`。
...
1. **REST** — 调用一次 `POST /user/login`，缓存令牌，然后调用 `GET /user/getMe` 和 `GET /games/v2/leagues` 以引导状态。
