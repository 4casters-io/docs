...
<Note>
更新的 gRPC 變體位於 `POST /session/v4/editOrder`。它接受相同的請求體，並在校驗失敗時額外返回 `errorType`。確認形態適合你後，新整合請優先使用它。
</Note>
...
結果映象 [下單](/zh-Hant/pages/rest/orders/place-order#response) 的響應形態 — 一個 `cancelled` 彙總，加上新的 `matched` / `unmatched` 結果：
...
| 狀態 | 含義 |
| ------ | ------- |
| `400`  | 校驗錯誤（`v4/editOrder` 會為這些錯誤返回 `errorType`）。 |
| `503`  | 訂單服務不可達。 |
