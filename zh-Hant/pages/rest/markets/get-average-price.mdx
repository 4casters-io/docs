...
返回某聯賽中每場賽事的最優價與均價的截斷列表。對每場賽事和每個市場（moneyline / spread / total），響應包含：
...
<Note>
與大多數端點不同，此端點在**頂層**返回賽事（不在 `data` 下）。
</Note>
  ...
  <Expandable title="單場附加欄位">
    ...
    <ResponseField name="odds" type="object">
      ...
      <Expandable title="odds">
        ...
        <ResponseField name="bestHomeMoneylinePriceLimit"     type="integer">主勝單勝盤最優價下的最大投注額。</ResponseField>
