...
返回交易所範圍內去重後的參賽方列表 — 球隊、球員、期貨盤方向。下 `moneyline` 或 `spread` 訂單時，將此端點的 `participantID` 值用作 `side` 欄位。
...
<ParamField query="active" type="string">
  當 `active=true` 時，將響應限定為未來開賽賽事的參賽方。當 `active=false`（或省略）時，返回所有賽事的參賽方。
</ParamField>
