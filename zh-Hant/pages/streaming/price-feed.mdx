...
預設情況下，已連線的客戶端會收到所有運動專案和聯賽的全部市場更新。若要收窄全量推送，可在套接字進入 `open` 之後，以 JSON 文字幀傳送下列命令之一。命令在該連線的生命週期內生效。

從未傳送過訂閱命令的客戶端保持全量廣播模式，因此現有整合無需改動即可繼續工作。

### 按 gameID、聯賽或運動專案訂閱 {#subscribe-by-gameid-league-or-sport}

安裝過濾器。`gameIDs` 為 24 位 Mongo ObjectID 十六進位制字串；`leagueIDs` 為短程式碼，如 `NBA` / `MLB` / `EPL`；`sportIDs` 為小寫標識，如 `basketball` / `baseball` / `soccer`。
...
- 運動專案 ID 不區分大小寫 — `baseball` 與 `Baseball` 等價。
- 陣列中的空字串會被忽略。
- `subscribe` 在 `replace: false` **且** 陣列全部為空時為無操作（有缺陷的客戶端不會因傳送空的追加訂閱而意外把自己訂空）。
...
任意市場中的訂單發生變化時發出（新建、修改、成交或取消）。載荷僅包含接收使用者有權看到的 `sideOrders`（自己的訂單以及可成交的對手方）。
...
對於 `moneyline1x2`（足球三項盤：主勝 / 平局 / 客勝），投注物件由 **`market` + `side` 組合**標識，而不是 `participantID` / `OU`：

- `market` — 該盤口所針對的結果：參賽者 ObjectID 十六進位制（主隊或客隊），或字面字串 `"draw"`。
- `side` — 對該結果的 `"yes"` 或 `"no"`。
- `OU`、`total` 和 `spread` 不出現，`participantID` 為空 — 請使用 `market` + `side`。

這兩個欄位既出現在更新層級（標識哪一側訂單簿發生了變化），也出現在 `sideOrders` 的每一條記錄上。
...
賽事被建立或其狀態發生變化時發出（盤口開盤、收盤、開賽時間更新）。載荷為完整渲染後的賽事物件。
