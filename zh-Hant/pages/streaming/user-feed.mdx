...
使用者推送將每條更新作為**裸 JSON 物件**傳送（無元組信封）。所有更新共用同一信封；動作由 `unmatched` / `matched` 哪個被填充以及 `origin` 決定。
...
| 欄位 | 型別 | 是否下發 | 含義 |
|---|---|---|---|
| `txID` | string | 始終 | 該注單的 id。用它做成交去重 — 與[獲取已撮合投注](/zh-Hant/pages/rest/user/get-matched-bets)返回的 `txID` 相同。 |
...
| `offered` | number | 始終 | 該訂單掛出的總風險。`0` 標識取消。 |
...
|---|---|---|---|
| `moneyline` | 參賽者 ObjectID 十六進位制 | — | — |
| `spread` | 參賽者 ObjectID 十六進位制 | 讓分盤口（例如 `6.5`） | — |
| `total` | `"over"` / `"under"` | 大小球盤口（例如 `209.5`） | — |
| `moneyline1x2` | `"yes"` / `"no"` | — | 參賽者 ObjectID 十六進位制（主隊或客隊）或 `"draw"` |
...
足球三項盤上的成交攜帶上文所述的 **`market` + `side` 組合**，而不是參賽者十六進位制 `side`。此處使用者以 +120 吃了主隊的 "yes"（投注阿森納獲勝）：
...
鑑權方式與 WebSocket 相同：在 `Authorization` 請求頭、`Auth` 請求頭或 `token` 查詢引數中攜帶 token。僅返回屬於已鑑權使用者的訊息。
...
### 響應：`success` {#response-success}

你的 `afterID` 仍在保留歷史中，因此返回的列表是**完整的**缺口填充 — `afterID` 與 `beforeID` 之間沒有任何丟失：
...
### 響應：`cache_expired` {#response-cache_expired}
...
### 示例：帶回放的重連客戶端 {#example-reconnecting-client-with-replay}

完整的 Node.js 客戶端（為簡潔起見省略 keepalive ping/pong — 參見本頁頂部的快速入門）。正確性依賴兩點：每條訊息 — 即時、緩衝或回放 — 都經過同一套只向前推進的遊標檢查；回放失敗時重連，而不是帶著缺口進入即時。
