...
專用 API 金鑰已在路線圖中。釋出時會在 [更新日誌](/zh-Hant/pages/changelog) 中公告，本頁也會隨之更新。在此之前，你的登入就是憑據。
</Warning>

## 工作原理 {#how-it-works}

1. 透過 REST API 的 `POST /user/login`，使用帳戶的使用者名稱（或郵箱）和密碼**登入**。
2. 在回應中**收到一個令牌**。它是一個 256 個字元的十六進位制字串。
3. 在每個 REST 請求和每次 WebSocket 握手中**傳送該令牌**。三套 API 接受同一個令牌。

API 訪問沒有單獨的註冊流程。只要能在網站上登入，就能使用 API。
...
`username` 欄位既可以填使用者名稱，也可以填帳戶的郵箱。
...
<Tip>
**只登入一次**，然後複用令牌。登入按 IP 和按帳戶限流，且每次成功登入都會簽發一個新令牌。再次登入不會使你已持有的令牌失效。
</Tip>
...
<Tabs>
  <Tab title="REST API">
    在 `Authorization` 請求標頭中傳遞令牌。`Bearer ` 字首可選。
...
一個只登入一次、儲存令牌、從不檢視回應標頭的指令碼，會恰好執行 30 天，然後每個請求都以 `401` 失敗。

## 雙重認證 {#two-factor-authentication}

4casters 的雙重認證保護的是**提現**，而不是登入。在帳戶上啟用它不會改變你對 API 的認證方式。`POST /user/login` 從不要求驗證碼。
...
| `428` | `CHALLENGE_REQUIRED`。僅在平臺正在主動抵禦撞庫攻擊時回傳；此時要求完成人機驗證。 | 繼續使用已持有的令牌，或稍後重試。指令碼客戶端無法完成該驗證。 |
| `429` | 超出登入限流。 | 按 `Retry-After` 指定的秒數退避，並複用現有令牌而不是重新登入。 |
