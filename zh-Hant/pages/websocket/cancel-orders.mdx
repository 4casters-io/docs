...
支援四種取消方式。它們都使用相同的響應信封：`{ requestID, data: [ ...CancelResponsePayload ] }`。

## 取消響應載荷 {#cancel-response-payload}

每次成功取消都會返回一條或多條如下形態的記錄。注意請求與響應的欄位大小寫不同：請求為 `orderId`/`gameId`（`d` 小寫），響應為 `orderID`/`gameID`（`ID` 大寫）。
...
當取消無法處理時，伺服器返回標準錯誤信封（無 `data` 欄位）：
...
對於批次取消（`cancelMultiple`、`cancelAllByGame`、`cancelAll`），單筆失敗會出現在 `data` 中，帶有 `success: false` 以及解釋原因的 `description` — 只有信封級的 `error` 才表示整個請求失敗。
